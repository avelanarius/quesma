	...
	for _, command := range commands {
		...
		if len(fields) > 0 {
			...
			case "select":
				...
				currentSQL = fmt.Sprintf("%s FROM (%s)", command, currentSQL)

			case "where", "group", "order":
				startSQL()

				currentSQL = fmt.Sprintf("SELECT * FROM (%s) %s", currentSQL, command)
			...
			case "aggregate":
				...
				case 2:
					currentSQL = fmt.Sprintf("SELECT %s, %s  FROM (%s) GROUP BY %s", ary[0], ary[1], currentSQL, ary[1])
			...
			case "???":
				...
				currentSQL = fmt.Sprintf("SELECT * FROM (%s) %s", currentSQL, "LIMIT 100")
